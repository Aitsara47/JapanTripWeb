...
place_data.map(
    (item,index)=>{
        const main_div = document.createElement("a");
        main_div.href = `./product.html?id=${index}`;
        // main_div.onclick = ()=>{window.location.href = "http://www.w3schools.com";}
        main_div.classList.add("items");
        ...
        Group.innerText = `Group Size: ${item.Group}`;
        
        const Price = document.createElement("h4");
        Price.innerText = `Price: ฿${item.Price}`;

        //button_container last
        const button_container = document.createElement("div");
        button_container.classList.add("all-button");

 

        const Submit_button = document.createElement("button");
        Submit_button.classList.add("add-to-cart");
        Submit_button.innerText = "Add to cart";

        button_container.appendChild(Submit_button);

        main_div.appendChild(img);
        main_div.appendChild(name);
        main_div.appendChild(duration);
        main_div.appendChild(Group);
        main_div.appendChild(Price);
        main_div.appendChild(button_container);
...
document.getElementById("search_button").addEventListener("click",(e)=>{
    ...
    const newKW = String(keywords).replaceAll(" ","").trim();

    const filter_data = place_data.filter((e)=>{return (e.name.replaceAll(" ","").trim()).includes(newKW)});
